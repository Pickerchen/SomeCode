...
import com.sen5.ocup.gui.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener, OnScrollListener {
	...
	public void notifyDidRefresh(boolean isNoMoreData) {
		...
		mIsNoMoreData = isNoMoreData;
		updateFooter();
	//	System.out.println("底部加载更过"+mIsNoMoreData);
	...
	private void updateFooter() {
		Log.d("适配器","更新底部视图"+mEnableLoadMore);
		if (!mEnableLoadMore) return;//如果为false就直接返回
